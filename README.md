...
3. `vlahost` server running on the robot and exposing `GET /state` and `POST /action`
...
On the robot machine, build and start vlahost (requires ROS 2):
...
4. Start `vlahost` on the robot (`ros2 launch vlahost vlahost_server.launch.py`)
